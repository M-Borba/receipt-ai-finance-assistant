...
| **Las deudas de a pares se reparten con un problema de transporte, no fila por fila** | Repartir la deuda de cada deudor por separado con el resto mayor da las filas bien y **las columnas mal**: con dos deudores de un centavo y dos acreedores de un centavo, los dos le dan su centavo al mismo acreedor. Medido: columnas equivocadas en el 83% de los casos con más de un pagador. Y se ve en pantalla, porque el saldo neto de arriba contradice las deudas de abajo. `allocateDebtsToCredits` usa el método clásico de transporte (parte entera de la proporción exacta, y después los centavos sobrantes a las celdas de mayor resto que todavía tengan lugar en su fila **y** en su columna): las dos márgenes cierran exactamente y nunca da negativos. |
| **El deep link viaja en `?from=` hasta después de autenticarse** | El link de invitación solo funcionaba para quien **ya tenía la sesión abierta**. A quien no, y es el caso normal porque lo estás invitando justamente porque todavía no está, el `redirect` lo mandaba a `/auth/login` pelado: el destino se perdía ahí, y después de crearse la cuenta caía en el dashboard sin grupo y sin ningún mensaje. Ahora el destino viaja en el query `from` por el splash, el login y el registro. La decisión entera vive en `decidirRedireccion`, lógica pura sin Flutter, porque el bug era **una línea dentro de un closure** y no había forma de escribirle un test. El `from` viene de la URL, así que se valida: `//dominio` y `/\dominio` los leen los navegadores como otro sitio, y aceptarlos convertiría cualquier link de la app en un redirect abierto. |
